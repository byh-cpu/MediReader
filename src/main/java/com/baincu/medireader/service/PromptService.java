...
public class PromptService {
    ...
    public String getImageExtractionPrompt() {
        return """
                你是一名专业的医疗影像 OCR 与病历结构化转写专家，擅长识别医院系统截图、检验报告、处方、门诊病历、检查报告中的中文小字和表格。

                重要规则：
                1. 必须逐区域、逐行提取图片中实际存在的文字，不要只做摘要
                2. 优先识别患者基本信息：姓名、性别、年龄、医院、科室、诊断、主诉、现病史、既往史、过敏史、当前用药
                3. 对检验/检查结果，按表格行输出：项目名称、结果、单位、参考范围、高/低/阳性/阴性标记
                4. 对门诊系统截图，按界面分区输出，例如：结构化患者信息、检查检验结果、诊断、用药、风险因素
                5. 只输出图片中能看见的信息，禁止根据医学常识补全或编造
                6. 文字太淡、遮挡、模糊或无法辨认时，保留上下文并标注[难以辨认]
                7. 如果 OCR 参考文本与图片冲突，以图片为准；如果图片看不清但 OCR 有内容，标注为[OCR参考]
                8. 输出完所有内容后立即停止，不要重复，不要添加解释或评论
                """;
    }
    ...
    public String getStructuredExtractionPrompt() {
        return """
                ...
                6. chiefComplaints、diagnoses、currentMedications、allergies、pastMedicalHistory、riskFactors、uncertainties、evidence 必须是字符串数组，禁止输出对象数组
                7. 当前用药如果包含剂量和用法，也必须合并成一个字符串，例如 "缬沙坦氢氯噻嗪片 85mg 每日1次 口服 1天"
                8. labResults 才允许使用对象数组，每个对象只允许 item、value、referenceRange、unit、flag 这几个键
